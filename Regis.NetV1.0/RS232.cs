...
namespace Regis2008
{
    ...
    public abstract class CommBase : IDisposable
    {
        ...
        public class CommBaseSettings
        {
            ...
            /// <param name="StopBitsMode"></param>
            /// <param name="ParityMode"></param>
            public void SetStandard(string Port, int Baud, Handshake Hs, int DataBits, StopBits StopBitsMode,
                                    Parity ParityMode)
            {
                dataBits = DataBits;
            ...
            {
                var sr = new XmlSerializer(GetType());
                sr.Serialize(s, this);
            }
        ...
        public bool Open()
        {
            var PortDCB = new Win32Com.DCB();
            var CommTimeouts = new Win32Com.COMMTIMEOUTS();
            CommBaseSettings cs;
            var wo = new Win32Com.OVERLAPPED();
            ...
            if (hPort == (IntPtr)Win32Com.INVALID_HANDLE_VALUE)
            {
                ...
                else
                {
                    ...
                    hPort = Win32Com.CreateFile(AltName(cs.port), Win32Com.GENERIC_READ | Win32Com.GENERIC_WRITE, 0,
                                                IntPtr.Zero,
                                                Win32Com.OPEN_EXISTING, Win32Com.FILE_FLAG_OVERLAPPED, IntPtr.Zero);
                    if (hPort == (IntPtr)Win32Com.INVALID_HANDLE_VALUE)
                    {
...
#pragma warning restore 618,612
            ...
            rxThread = new Thread(ReceiveThread);
        ...
        public struct QueueStatus
        {
            ...
            public override string ToString()
            {
                ...
                {
                    m.Append(inQueueSize + " bytes long and ");
                }
                if (inQueue == 0)
                {
                    m.Append("is empty.");
                ...
                {
                    m.Append(outQueueSize + " bytes long and ");
                }
                if (outQueue == 0)
                {
                    m.Append("is empty");
        ...
        private void ReceiveThread()
        {
            ...
            var ov = new Win32Com.OVERLAPPED();
...
#pragma warning restore 618,612
            ...
            try
            {
                ...
                {
                    if (
                        !Win32Com.SetCommMask(hPort,
                                              Win32Com.EV_RXCHAR | Win32Com.EV_TXEMPTY | Win32Com.EV_CTS |
                                              Win32Com.EV_DSR
        ...
        private bool CheckOnline()
        {
            ...
            if (online)
            {
                ...
                if (hPort != (IntPtr)Win32Com.INVALID_HANDLE_VALUE) return true;
    ...
    public abstract class CommLine : CommBase
    {
        ...
        public class CommLineSettings : CommBaseSettings
        {
            ...
            /// </summary>
            public int rxStringBufferSize = 13;
            ...
            /// </summary>
            public int transactTimeout = 1500;
        ...
        protected void Send(string toSend)
        {
            ...
            var l = (uint)Encoding.ASCII.GetByteCount(toSend);
    ...
    internal class Win32Com
    {
        ...
        [DllImport("kernel32.dll")]
        internal static extern Boolean BuildCommDCBAndTimeouts(String lpDef, ref DCB lpDCB,
    ...
    internal class RS232 : CommLine
    {
        public Pirometer pirometer
        {
            get { return pirometer_; }
        }
        ...
        public byte[] receivedTemp = new byte[13];
        public byte[] receivedMem = new byte[32037];
        private readonly Pirometer pirometer_;
        public int Baud = 9600;
        public int DataBits = 8;
        private string visibleName = "Not defined";
        private string systemName = "COM1";
        public Handshake HandshakeMode = Handshake.none;
        public Parity ParityMode = Parity.none;
        public StopBits StopBitsMode = StopBits.two;


        public delegate void EventHandler(object sender, EventArgs e);

        // public event EventHandler DataReceived;
        public event EventHandler ByteReceived;
        ...
        {
            receivedChar = ch;
            ByteReceived(this, new EventArgs());
        }
        ...
        protected override void BeforeClose(bool error)
        {
            ...
            base.BeforeClose(error);
        }

        //internal string buffer;
        internal byte receivedChar;


        protected override CommBaseSettings CommSettings()
        {
            var cs = new CommLineSettings();
            cs.SetStandard(SystemName, Baud, HandshakeMode, DataBits, StopBitsMode, ParityMode);
            cs.stopBits = StopBitsMode;
